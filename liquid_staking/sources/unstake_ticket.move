...
module liquid_staking::unstake_ticket {
    ...
    // created once
    public(friend) fun create_metadata(ctx: &mut TxContext): Metadata {
        Metadata {
            id: object::new(ctx),
            total_supply: 0,
            version: VERSION,
    ...
    public(friend) fun get_max_supply_for_2epochs(self: &Metadata, ctx: &mut TxContext): u64 {
    ...
    public(friend) fun wrap_unstake_ticket(metadata: &mut Metadata, value: u64, unstake_fee: u64, unlocked_in_epoch: u64, ctx: &mut TxContext): UnstakeTicket {
    ...
    // destroy ticket
    public(friend) fun unwrap_unstake_ticket(metadata: &mut Metadata, ticket: UnstakeTicket, ctx: &TxContext): (u64, u64) {
        let UnstakeTicket {
            id,
            unlock_epoch: _,
            value,
    ...
    public(friend) fun transfer(ticket: UnstakeTicket, to: address) {
